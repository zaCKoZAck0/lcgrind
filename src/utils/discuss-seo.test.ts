...
const base = {
    url: "https://example.com/discuss/abc123-my-post",
...
describe("buildDiscussionForumPostingJsonLd", () => {
    it("produces a DiscussionForumPosting with month-only dates", () => {
        const ld = buildDiscussionForumPostingJsonLd(base);
    ...
    it("emits dateModified only when edited in a different month", () => {
        const same = buildDiscussionForumPostingJsonLd({
            ...base,
            editedMonth: "2026-03",
        });
        expect(same.dateModified).toBeUndefined();

        const edited = buildDiscussionForumPostingJsonLd({
            ...base,
            editedMonth: "2026-05",
        });
    ...
    it("leaks no provenance (no discuss urls, no email)", () => {
        const json = JSON.stringify(buildDiscussionForumPostingJsonLd(base));
