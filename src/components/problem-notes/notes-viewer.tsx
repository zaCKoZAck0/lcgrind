    ...
    <Dialog>
      ...
      <DialogTrigger asChild>
        <Button
          variant="reverse"
          size="sm"
          className="gap-1 cursor-pointer relative h-fit bg-secondary-background text-secondary border-secondary/20 px-1"
          title={`${notes.length} note${notes.length !== 1 ? "s" : ""}`}
        >
          <PlusIcon size={14} /> <span className="hidden md:inline">Notes</span>
          {notes.length > 0 && (
        ...
        <div className="overflow-y-auto max-h-[60vh] space-y-4">
          {notes.length === 0 ? (
            <div className="text-center py-8 text-foreground">
              <PlusIcon size={48} className="mx-auto mb-4" />
              <p className="text-lg font-medium">No notes yet</p>
              <p className="text-sm">
                Click &quot;Add Note&quot; to create your first note for this
                problem.
              </p>
            </div>
          ) : (
